...
This feature focuses exclusively on single-workflow message flow validation (producer → Kafka → consumer, coordinated by the orchestrator). The following are **explicitly out of scope**:
- Security testing (authentication, authorization, encryption)
- Performance benchmarking beyond the defined success criteria
- Concurrent multi-workflow scenarios
- UI integration testing

These exclusions may be addressed in separate future features.
...
As a developer, I want to verify that the orchestrator correctly coordinates producer and consumer activities — including starting, monitoring, and stopping workflows — so the full orchestration lifecycle is reliable. Tests use a mock orchestrator implementing the same interface as the real orchestrator (specs/003-workflow-definition/) to simulate production behavior.
...
- How are orphaned test topics cleaned up from aborted test runs — tests auto-delete leftover `test.*` topics at startup, and a manual cleanup script is provided for CI.
...
- **FR-007**: Tests MUST verify the orchestrator correctly starts and stops producer/consumer activities as part of workflow execution, using a mock orchestrator that implements the same interface as the real orchestrator.
- **FR-008**: Tests MUST verify the orchestrator handles workflow cancellation, including stopping producer publishing and consumer processing, through the mock orchestrator interface.
...
- Q: What is explicitly out of scope for this test feature? → A: Exclude security, performance benchmarking, and concurrent multi-workflow scenarios; stay focused on single-workflow message flow validation.
- Q: Does the orchestrator service already exist or should tests provide one? → A: Mock the orchestrator interface — tests use a mock implementing the same interface as the real orchestrator from specs/003-workflow-definition/ so behavior matches production.
- Q: What cleanup strategy should tests use for orphaned Kafka topics from aborted runs? → A: Auto-cleanup on each test run start (delete leftover `test.*` topics before creating new ones), plus a separate manual cleanup script for CI.
- Q: Should tests use a single consumer instance or both consumer1 and consumer2? → A: Single consumer (consumer1 only) — consumer group rebalancing is an infrastructure concern outside this feature's scope.
...
- Integration tests run against a single consumer instance (consumer1) to keep the test topology simple and avoid consumer group rebalancing timing concerns.
